...
class TripRepository(private val pool: PgPool) {
  ...
  fun getPoints(routeId: Int): Future<List<Point>> {
    ...
    pool.preparedQuery(
      "SELECT * FROM point WHERE route_id=$1",
      Tuple.of(routeId)
    ) { ar ->
      if (ar.succeeded()) {
        val rows = ar.result()
  ...
  private fun insertRoute(routeName: String): Future<Int> {
    ...
    pool.preparedQuery(
      "INSERT INTO route (name) VALUES($1) RETURNING id_route",
      Tuple.of(routeName)
    ) { ar ->
      if (ar.succeeded()) {
  ...
  fun insertCoordinates(points: JsonArray, routeId: Int): Future<JsonObject>? {
    ...
    while (i < points.size()) {
      ...
      pool.preparedQuery(
        "INSERT INTO point (order_position, coordinates, route_id) " +
          "VALUES($1, $2, $3)",
        Tuple.of(order, coordinates, routeId)
      ) { ar ->
        if (ar.succeeded()) {
          promise.complete()
        } else {
  ...
  fun insertTrip(
    ...
    pool.preparedQuery(
      ...
      Tuple.of(user.id, cost, description, peopleLimit, dateOffset, routeId, active)
    ) { ar ->
      if (ar.succeeded()) {
        promise.complete(
          TripDto(
  ...
  fun updateCoordinate(points: JsonArray, tripId: Int): Future<Nothing> {
    ...
    while (i < points.size()) {
      pool.preparedQuery(
        "UPDATE point SET coordinates = $1 " +
          "WHERE point.route_id = (SELECT route_id FROM trip WHERE id_trip = $2) AND order_position =$3",
        Tuple.of(
          points.getJsonObject(i).getString("coordinates"), tripId,
          points.getJsonObject(i).getInteger("order")
        )
      ) { ar ->
        if (ar.succeeded()) {
          promise.complete()
  ...
  fun deleteUserTrip(email: String, tripId: Int): Future<JsonObject> {
    val promise = Promise.promise<JsonObject>()
    pool.preparedQuery(
      "DELETE FROM trip_user_account t WHERE t.trip_id = $1 " +
