pipeline {
  ...
  stages {
    ...
    stage('Docker run phase test') {
      ...
      steps {
        sh 'docker run -d -p 8091:8090 -e DATABASE_HOST -e DATABASE_NAME -e DATABASE_USER -e DATABASE_PASSWORD --name backendruntest --net container:1641e1e677d6 -it backendtest'
      }
    }
    ...
    stage('Docker run') {
      ...
      steps {
        sh 'docker run -d -p 8090:8090 -e DATABASE_HOST -e DATABASE_NAME -e DATABASE_USER -e DATABASE_PASSWORD --name backendrun --net container:1641e1e677d6 -it backend'
      }
    }
