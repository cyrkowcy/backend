pipeline {
  ...
  stages {
    ...
    stage('Docker run phase test') {
      ...
      steps {
        sh 'docker run -d -p 8091:8090 -e DATABASE_HOST=172.18.0.4:5432 -e DATABASE_NAME -e DATABASE_USER -e DATABASE_PASSWORD -e APP_SECRET --name STAGE=STA backendruntest --restart always --net netapp -it backendtest'
      }
    }
    ...
    stage('Docker run') {
      ...
      steps {
        sh 'docker run -d -p 8090:8090 -e DATABASE_HOST=172.18.0.4:5432 -e DATABASE_NAME -e DATABASE_USER -e DATABASE_PASSWORD -e APP_SECRET --name STAGE=PRO backendrun --restart always --net netapp -it backend'
      }
    }
